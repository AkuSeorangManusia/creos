...
// calculated based on window size with a right margin
...
export default function Desktop() {
    ...
    const openApp = useCallback(
        (appLabel) => {
            ...
            const appWindowCount = windows.filter(
                (w) => w.appId === app.id,
            ).length;
    ...
    const openArticle = useCallback(
        (slug, articleTitle) => {
            ...
            const centerX = Math.max(
                10,
                (window.innerWidth - articleWidth) / 2,
            );
            const centerY = Math.max(
                60,
        ...
        >
            ...
            {windows.map((window) => (
                <Window
                    ...
                    onMove={(newPosition) =>
                        handleWindowMove(window.id, newPosition)
                    }
