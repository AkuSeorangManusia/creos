      ...
      <div className="flex items-center gap-3 mb-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1">
            <span className={`w-2 h-2 rounded-full ${statusColors[presence.discord_status]}`}></span>
            <span className="text-gray-600 capitalize">{presence.discord_status}</span>
          </div>
        </div>
        {/* Discord Logo */}
...
export function SteamProfileWidget({ onHeightChange }) {
  ...
  useEffect(() => {
    ...
    fetchSteamProfile();
    const interval = setInterval(fetchSteamProfile, 30000); // Refresh every 30 seconds
    return () => clearInterval(interval);
  }, []);
  ...
  useEffect(() => {
    ...
    let requiredHeight = 80; // Base height for avatar and username

    if (profile.gameextrainfo) {
      requiredHeight += 50; // Currently playing section
    }

    if (profile.recentGames?.length > 0 && !profile.gameextrainfo) {
      requiredHeight += 60; // Recent games section
    }
  ...
  return (
    <div className="flex flex-col h-full text-black text-xs">
      ...
      <div className="flex items-center gap-3 mb-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="font-bold text-sm truncate">{profile.personaname}</div>
          <div className="flex items-center gap-1">
            <span className={`w-2 h-2 rounded-full ${status.color}`}></span>
            <span className="text-gray-600 capitalize">{status.label}</span>
          </div>
        </div>
        {/* Steam Logo */}
      ...
      {isPlaying && (
        <div className="mb-2 pb-2 border-b border-gray-300">
          <div className="flex items-center gap-1 mb-1">
            <span className="text-green-600 font-bold">🎮</span>
            <span className="font-semibold text-green-700">Now Playing</span>
          </div>
          <div className="font-bold truncate">{profile.gameextrainfo}</div>
        </div>
      )}

      {/* Recent Games (if not currently playing) */}
      ...
      {/* If offline and no recent games */}
      {profile.personastate === 0 && profile.recentGames?.length === 0 && (
